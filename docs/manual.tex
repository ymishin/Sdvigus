...
\usepackage{textcomp}
%\usepackage[text={15cm,26cm},centering]{geometry}
...
%\item \textsl{Optional}: SuiteSparse package for better performance (although speedup is not dramatic). SuiteSparse will be used automatically if installed and can be found in the Matlab path.
...
%\item \textbf{Usage description (formal)}
\item \textbf{Formal description}

%The installation directory contains three functions which can be run either from Matlab Command Window, or in Matlab batch mode (using Matlab %\texttt{-r} argument). These functions are:
%These scripts can be run either from Matlab Command Window, or in Matlab batch mode (using Matlab \texttt{-r} %argument). The syntaxes for these functions is the following:
%\texttt{\$SDHOME}:
%The following MATLAB functions can be found at the top level of the installation directory:
...
The following is the demonstration how to run the code for the Rayleigh-Taylor instability model provided in the directory \texttt{examples/rt} (Linux environment is assumed here, under Windows use \texttt{\symbol{`\\}} instead of \texttt{/}).
